"""One-off AuthRing ring IMU processor.

Output follows the repository processed-data format:
  /Volumes/Felix_Backups/Processed/sources/authring/segments/{mode}/ring/*.npz
  /Volumes/Felix_Backups/Processed/sources/authring/manifests/ring_{mode}.jsonl
"""
...
DATASET_ROOT = Path("/Volumes/Felix_Backups/Root/科研与科创/数据记录/AuthRing/Dataset")
...
class SkipSession(Exception):
    """Raised when a session should not produce ring output."""


def parse_args() -> argparse.Namespace:
    """Read command-line arguments for either one session or the whole Dataset."""

    parser = argparse.ArgumentParser(description="Process AuthRing ring IMU into NPZ training segments.")
...
def discover_sessions(dataset_root: Path) -> list[Path]:
    """Find all session directories in Dataset/participant/session format."""
...
def legacy_manifest_path(processed_root: Path, mode: str) -> Path:
    """Return the old source manifest path used before device-specific manifests."""

    return source_root(processed_root) / "manifests" / f"{mode}.jsonl"


def prepare_output(processed_root: Path, overwrite: bool) -> None:
    """Create ring output folders; optionally rebuild existing ring outputs."""

    ring_dirs = [segment_dir(processed_root, mode) for mode in MODES]
    manifests = [manifest_path(processed_root, mode) for mode in MODES]
    legacy_manifests = [legacy_manifest_path(processed_root, mode) for mode in MODES]

    if overwrite:
        for path in ring_dirs:
            if path.exists():
                shutil.rmtree(path)
        for path in manifests + legacy_manifests:
            if path.exists():
                path.unlink()
    else:
        existing = [str(path) for path in ring_dirs + manifests + legacy_manifests if path.exists()]
        if existing:
...
def read_session_json(session_dir: Path) -> dict:
    """Read session.json and return only fields needed for filtering and label."""

    data = json.loads((session_dir / "session.json").read_text(encoding="utf-8"))
    return {
        "ring_device_id": str(data["ringDeviceId"]),
        "finger_raw": str(data["ringWearingFinger"]),
    }


def is_bad_ce56(ring_device_id: str) -> bool:
    """Return whether this session used the excluded CE56 ring."""

    return "BCL603CE56" in ring_device_id or "F8:BC:9A:CD:CE:56" in ring_device_id


def make_label(finger_raw: str) -> dict:
    """Create the label object; keep only the English finger label."""
...
def read_ring_csv(session_dir: Path) -> np.ndarray:
    """Read ring_imu.csv as float64 and drop non-finite rows."""
    ...
    try:
        ...
        raise SkipSession(f"cannot parse ring_imu.csv: {exc}") from exc
    ...
    data = data[:, : len(RING_COLUMNS)]
    data = data[np.all(np.isfinite(data), axis=1)]
    if len(data) == 0:
        raise SkipSession("no finite ring rows")
    return data


def clean_ring_timeline(data: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Convert ring ticks to ms, sort by time, and keep first duplicate."""
...
def process_session(session_dir: Path, processed_root: Path, writers: dict, counters: dict, summary: dict) -> dict:
    """Process one session and directly write all passing ring segments."""

    before = summary["segments"].copy()
    meta = read_session_json(session_dir)
    if is_bad_ce56(meta["ring_device_id"]):
        summary["skipped_ce56"] += 1
        raise SkipSession("CE56 ring excluded")

    label = make_label(meta["finger_raw"])
...
def run(dataset_root: Path | None, session_dir: Path | None, processed_root: Path, overwrite: bool) -> dict:
    """Run the ring processing pipeline."""
    ...
    with manifest_path(processed_root, "10s").open("w", encoding="utf-8") as man_10s, manifest_path(
        ...
        for idx, session in enumerate(sessions, start=1):
            try:
                ...
                reason = str(exc)
                if reason == "CE56 ring excluded":
                    pass
                elif reason == "raw average hz below 150":
                    summary["skipped_low_hz"] += 1
                else:
                    summary["skipped_other"] += 1
                summary["skip_reasons"][reason] = summary["skip_reasons"].get(reason, 0) + 1
                print(f"[{idx}/{total}] {session.name} SKIP {reason}", flush=True)
