"""One-off AuthRing phone IMU processor.

Output follows the repository processed-data format:
  /Volumes/Felix_Backups/Processed/sources/authring/segments/{mode}/phone/*.npz
  /Volumes/Felix_Backups/Processed/sources/authring/manifests/phone_{mode}.jsonl
"""
...
class SkipSession(Exception):
    """Raised when a session should not produce phone output."""


def parse_args() -> argparse.Namespace:
    """Read command-line arguments for either one session or the whole Dataset."""

    parser = argparse.ArgumentParser(description="Process AuthRing phone IMU into NPZ training segments.")
...
def prepare_output(processed_root: Path, overwrite: bool) -> None:
    """Create phone output folders; optionally rebuild existing phone outputs."""
...
def read_session_json(session_dir: Path) -> dict:
    """Read session.json and return the finger label needed for manifest rows."""

    data = json.loads((session_dir / "session.json").read_text(encoding="utf-8"))
    return {"finger_raw": str(data["ringWearingFinger"])}
...
def read_phone_csv(session_dir: Path, filename: str) -> np.ndarray:
    """Read a phone IMU CSV as float64 and drop non-finite rows."""
    ...
    if len(data) == 0:
        raise SkipSession(f"no finite rows in {filename}")
    return data
...
def process_session(session_dir: Path, processed_root: Path, writers: dict, counters: dict, summary: dict) -> dict:
    """Process one session and directly write all passing phone segments."""
...
def run(dataset_root: Path | None, session_dir: Path | None, processed_root: Path, overwrite: bool) -> dict:
    """Run the phone processing pipeline."""
